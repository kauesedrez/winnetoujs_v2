...
const drawFinal = () => {
  ...
  if (contadorDeWarnings > 0) {
    drawText(
      "... with " + contadorDeWarnings + " warnings"
    );
    drawBlankLine();
  }
...
try {
  ...
  drawError(
    "Configuration file (winConfig.json) error or missing; original error: " +
      e.message
  );
  return;
}
...
if (
  ...
  (!config.constructosUrl ||
    config.constructosUrl.length < 1)
) {
  drawError(
...
if (!config.builtIns.jquery)
  config.builtIns.jquery = "none";
...
if (!config.extras.minifyHTML)
  config.extras.minifyHTML = [];
...
if (config.livereload) {
  ...
  for (
    let i = 0;
    i < config.extras.minifyHTML.length;
    i++
  ) {
...
const PerformJs = async () => {
  ...
  // console.log(construtos);

  code["GENERATED_inlinejs/constructos.js"] = construtos;

  if (
    !fs.existsSync(
      path.join(__dirname, "/GENERATED_inlinejs")
    )
  ) {
    fs.mkdirSync(
      path.join(__dirname, "/GENERATED_inlinejs")
  ...
  fs.writeFileSync(
    path.join(
      __dirname,
      "/GENERATED_inlinejs/constructos.js"
    ),
    construtos
  );
   ...
   * Icones
   *
   * Se a promisse dos icones retornou ok já inicializa os icones no projeto.
   *
   */
  ...
  if (
    ...
    // possui ícones, inicializa
    drawAdd("Icons powered by WinnetouJs on fire!");
    code["icones"] = `W.create("winIcons", "body")`;
  }
  ...
  if (config.builtIns.jquery === "latest") {
    let jquery = await adicionarURLAoBundle(
  ...
  if (config.defaultLang) {
    ...
    if (!config.folderName) {
      drawError(
        "WinnetouJs Translation Miss Configuration Error: You have to specify the name of winnetou folder in order to use the translations; Tip: Define 'folderName' in winConfig.json file."
      );
    } else {
      code[
        "traducoes.js"
...
const BundleJs = async (dados) => {
  ...
  if (
    config.map === true ||
    typeof config.map === "undefined"
  ) {
    result = UglifyJS.minify(dados, {
      sourceMap: {
        root: `/${config.folderName}` || "/winnetoujs",
    ...
    if (
      ...
      if (config.icons && config.icons.length > 0) {
        for (let i = 0; i < config.icons.length; i++) {
          ...
          let icones = await adicionarIcones(
            config.icons[i]
          );
      ...
      if (
        ...
        for (
          let i = 0;
          i < config.coloredIcons.length;
          i++
        ) {
          let icones = await adicionarIconesColoridos(
            config.coloredIcons[i]
          );
          drawAdd(
            "Icones adicionados: " + config.coloredIcons[i]
          );
          constructoIcones += icones;
        }
      }
...
const adicionarConstrutosAoBundle = async () => {
  return new Promise(async (resolve, reject) => {
    ...
    if (
      ...
      for (
        let i = 0;
        i < config.constructosUrl.length;
        i++
      ) {
    ...
    jsarray.forEach((item) => {
      ...
      babel.transform(
        ...
        function (err, result) {
          if (err) {
            drawError(
              `Error on parsing inline JS Babel: ${err} \n ${item[0]}`
            );
          } else {
      ...
      try {
        babel.transform(
          ...
          function (err, result) {
            if (err) {
              drawError(
                `Error on add Winnetou Core: ${err}`
              );
              return reject(err);
            }
        ...
        drawError(
          `Error on add Winnetou Core: ${e.message}`
        );
        return reject(e.message);
      }
    });
      ...
      try {
        babel.transform(
          ...
          function (err, result) {
            ...
            if (err) {
              drawError(
                `\nError on parsing js in Babel\n${err}\n\nFile: ${arquivo}`
              );

              return reject("babel error");
            } else {
              return resolve({
                nome: arquivo,
                codigo: result.code,
...
const adicionarIconesColoridos = async (path) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let a = 0; a < files.length; a++) {
        let svgName = await adicionarSvgColorido(
          path + "/" + files[a]
        );
