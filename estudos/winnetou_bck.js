...
class Winnetou {
  ...
  constructor(debug = "", next) {
    ...
    if (window.history && window.history.pushState) {
      window.onpopstate = function (event) {
        if ($debug == "debug") {
          console.warn(
            `popstate triggered: location: ${document.location}, state: ${JSON.stringify(
              event.state
            )}`
          );
        }
  ...
  callRoute(url) {
    ...
    try {
      ...
      $this.debug === "debug"
        ? console.error(
            `WinnetouJs Error, id: R001 \n\nThe provided URL was not found or the this.routes const not have been declared as a valid object of named functions. See WinnetouJs docs for more information. \n\nRoute: ${url} \n\nOriginal Error:`,
            e
          )
        : null;
      try {
  ...
  pushState(url) {
    try {
      ...
      this.log(5, "chamou o push state com sucesso", url);
    } catch (e) {
      history.pushState(url, null);
      this.log(5, "falha no pushstate", url, e);
    }
  }
  ...
  create(constructo = "", output = "", elements = {}, options = {}) {
    ...
    $log(6, "dentro do create", constructo, output, elements);
...
Output: ${output}               
    ...
    function imprime(opt = "") {
      $log(6, "dentro dentro do imprime");

      function geraIdentifier() {
        $log(6, "dentro do gera identifier");
        let identifier;
      ...
      el.forEach(item => {
        ...
        if (opt === "") {
          if ($vdom.substr(0, 3) === "<tr") {
            item.innerHTML = item.innerHTML + $vdom;
          } else {
            $log(6, "dentro appendChild");
   ...
   */
  lang(next) {
    // testa a definição do folderName
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        //$this.log(6, "frases", frases);
        next();
      }
    };
    xhttp.open("GET", `/${folderName}/translations/${defaultLang}.xml`, true);
    xhttp.send();
  }
